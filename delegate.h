...
class DxccDelegate : public QStyledItemDelegate {
public:
    using QStyledItemDelegate::QStyledItemDelegate;

    void setModeVisibility(const std::array<bool, 3> &visible)
    {
        modeVisible = visible;
    }
               ...
               const QModelIndex &index) const override
    {
        ...
        int bits = index.data(Qt::DisplayRole).toInt();
        const QStringList symbols = {"CW", "PH", "DG"};

        QVector<int> visibleIndices;
        visibleIndices.reserve(3);
        for (int i = 0; i < 3; ++i) {
            if (modeVisible[i]) {
                visibleIndices.push_back(i);
            }
...
private:
    std::array<bool, 3> modeVisible{{true, true, true}};
};
